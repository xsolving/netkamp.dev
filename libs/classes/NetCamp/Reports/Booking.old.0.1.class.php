...
class Booking{
    ...
    public function getBookingData($startDate, $endDate) {
	   ...
	   $retResult = array();
   	   if(! $stmtLodges =odbc_prepare($con, $getLodges)) {
			$this->outStatus->insertError( "Booking.class.php", "getBooking", "Prepare 1", "Errore query: ".getErrorMessage($stmtLodges), 100);
			return false;
	   } 
	   else {
	   	   odbc_execute( $stmtLodges ); 
	   	   //CReo l'array di output
	   	   $lodgesIdList=array();
	   	   while ($ris = odbc_fetch_array( $stmtLodges ) ) {
   	   ...
   	   //$con = $NetCampDB->getConnection($this->referralYear);
   	   //$retResult = array();
   	   if(! $stmt =odbc_prepare($con, $getData)) {

			$this->outStatus->insertError( "Booking.class.php",  "getBooking", "Prepare 1", "Errore getIdFromSession: ".getErrorMessage($stmt), 100);
